...
describe('ResumesService', () => {
  it('creates valid default content and maps persistence values', async () => {
    ...
    await expect(service.create(ownerId, { title: 'My CV' })).resolves.toMatchObject({
      id,
      title: 'My CV',
      content: createEmptyResumeContent(),
    });
    expect(resume.create).toHaveBeenCalledWith({
      data: { title: 'My CV', ownerId, content: createEmptyResumeContent(), template: 'classic' },
    });
  });
  it('lists newest resumes and validates stored JSON', async () => {
    const { service, resume } = makeService();
    await expect(service.list(ownerId)).resolves.toHaveLength(1);
    ...
    await expect(invalid.service.list(ownerId)).rejects.toThrow();
  });
  it('returns consistent not found errors for get, update, and delete', async () => {
    await expect(
      makeService({ findFirst: vi.fn().mockResolvedValue(null) }).service.get(ownerId, id),
    ).rejects.toMatchObject({ status: 404 });
    await expect(
      makeService({ updateMany: vi.fn().mockResolvedValue({ count: 0 }) }).service.update(
        ownerId,
        id,
        { title: 'New' },
      ),
    ...
    await expect(
      makeService({ deleteMany: vi.fn().mockResolvedValue({ count: 0 }) }).service.delete(
        ownerId,
        id,
      ),
    ).rejects.toMatchObject({ status: 404 });
